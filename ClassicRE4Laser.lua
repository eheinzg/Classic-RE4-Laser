...
if laser_trail_gameobject then
  pcall(function()
    laser_trail_gameobject:call("destroy")
  end)
...
if not enable_laser_trail or not re4.last_muzzle_pos or is_non_muzzle_weapon or not _G.is_aim or (simple_static_mode and not show_laser_dot) or has_stale_muzzle_data then
  if laser_trail_gameobject then
...
re.on_pre_gui_draw_element(function(element, context)
  ...
  if not show_laser_dot and enable_laser_trail and not (simple_static_mode and not show_laser_dot) then
    if reticle_names[name] then
      return false -- Hide reticle when laser is off, UNLESS simple static mode is enabled (then show white dot)
    end
  end
...
if reticle_names[name] then
  ...
  if type_panel then
    ...
    if current_color then
      ...
      if not is_non_muzzle_weapon then
        ...
        -- Handle dot visibility based on hotkey state
        if not show_laser_dot then
  ...
  if simple_static_mode and not show_laser_dot then
    -- Simple static mode with laser OFF: use basic game positioning for white backup dot
...
re.on_draw_ui(function()
  ...
  -- Weapon names array based on local RE4R_WeaponData.txt
  local weapon_names = {
      "SG-09 R",              -- 4000
      "Punisher",             -- 4001  
      "Red9",                 -- 4002
  ...
  for i, id in ipairs(weapon_ids) do
      local name = weapon_names[i] or ("Unknown Weapon")
